...
class EarlyStopping1:
    """Early stops the training if meta loss doesn't improve after a given patience."""
    def __init__(self, patience=10, verbose=False, delta=0, model_path='model_checkpoint.pth', trace_func=print):
        """
        Args:
            patience (int): How long to wait after last time meta loss improved.
                            Default: 10
        ...
        self.model_path = model_path
        self.trace_func = trace_func
    def __call__(self, meta_loss, model):

        score = -meta_loss

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(meta_loss, model)
        elif score < self.best_score + self.delta:
            self.counter += 1
            self.trace_func(f'EarlyStopping counter: {self.counter} out of {self.patience}')
            if self.counter >= self.patience:
    ...
    def save_checkpoint(self, meta_loss, model):
        '''Saves model when metaidation loss decrease.'''
        if self.verbose:
            self.trace_func(f'meta loss decreased ({self.meta_loss_min:.6f} --> {meta_loss:.6f}).  Saving model and vnet...')
        torch.save(model.state_dict(), self.model_path)

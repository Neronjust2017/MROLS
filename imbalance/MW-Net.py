...
# model and vnet path
model_path = results_path + 'models/model_state_imb_factor{}_seed{}.pth'.format(args.imb_factor, args.seed)
vnet_path = results_path + 'models/vnet_state_imb_factor{}_seed{}.pth'.format(args.imb_factor, args.seed)
weight_path = results_path + 'models/weight_imb_factor{}_seed{}.npy'.format(args.imb_factor, args.seed)
txt = results_path +  'results/result_imb_factor{}_seed{}.txt'.format(args.imb_factor, args.seed)

model_path_mid = results_path + 'models/model_state_imb_factor{}_seed{}_mid.pth'.format(args.imb_factor, args.seed)
vnet_path_mid = results_path + 'models/vnet_state_imb_factor{}_seed{}_mid.pth'.format(args.imb_factor, args.seed)

model_path_last = results_path + 'models/model_state_imb_factor{}_seed{}_last.pth'.format(args.imb_factor, args.seed)
vnet_path_last = results_path + 'models/vnet_state_imb_factor{}_seed{}_last.pth'.format(args.imb_factor, args.seed)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        indexes, weights = train(imbalanced_train_loader, validation_loader,model,vnet, optimizer_a, optimizer_c,epoch)
...
def train(train_loader, validation_loader,model, vnet,optimizer_a,optimizer_c,epoch):
    ...
    for i, (input, target, index) in enumerate(train_loader):
        ...
        # entropy = Categorical(probs=y_f_hat).entropy()
        # entropy = torch.reshape(entropy, (len(entropy), 1))
        cost_v = torch.reshape(cost, (len(cost), 1))

        # v_lambda = vnet(entropy)
        ...
        input_validation, target_validation, index = next(iter(validation_loader))

...
class CIFAR10(data.Dataset):
    ...
    def __init__(self, root='', train=True, meta=True, num_meta=1000,
                 ...
                 download=False, seed=1, train_labels='./results/targets_corruptionProb0.4_corruptionTypeflip2_trialFlip2.npy'):
        self.root = root
        ...
        # print(train, meta, np.random.randn())
        ...
        if self.train:
            ...
            if meta is True:
                ...
                self.train_labels_0 = copy.deepcopy(self.train_labels)
                ...
                if corruption_type == 'clabels':
                    ...
                    # np.save(train_labels, self.train_labels)
...
class CIFAR10_2(data.Dataset):
    ...
    def __init__(self, root='', train=True, corruption_prob=0, corruption_type='unif', transform=None, target_transform=None,
                 ...
                 train_labels='./results/targets_corruptionProb0.4_corruptionTypeflip2_trialFlip2.npy'):
        self.root = root
        self.transform = transform
        self.target_transform = target_transform
        self.train = train  # training set or test set
        ...
        if self.train:
            ...
            self.train_labels_0 = self.train_labels
            # if corruption_type == 'hierarchical':
            #     self.train_coarse_labels = list(np.array(self.train_coarse_labels)[idx_to_meta])
            ...
            if corruption_type == 'clabels':
                ...
                for i in range((len(self.train_labels) // 64) + 1):
                    current = self.train_data[i * 64:(i + 1) * 64]
                    current = [Image.fromarray(current[i]) for i in range(len(current))]
                    current = torch.cat([test_transform(current[i]).unsqueeze(0) for i in range(len(current))],
                                        dim=0)
                ...
                # for i in range(len(self.train_labels)):
                #     self.train_labels[i] = np.random.choice(num_classes, p=C[self.train_labels[i]])
                # self.train_labels = np.load('./results/targets_corruptionProb0.6_corruptionTypeunif.npy')
                self.train_labels = np.load(train_labels)
                self.corruption_matrix = C

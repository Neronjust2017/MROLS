...
class VCNN(MetaModule):
    def __init__(self, hidden1, output, k_size=5):
        super(VCNN, self).__init__()
        assert k_size % 2 == 1
        padding = (k_size - 1) // 2
        self.conv1 = MetaConv1d(1, 16, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.conv2 = MetaConv1d(16, 16, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.conv3 = MetaConv1d(16, 32, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.conv4 = MetaConv1d(32, 32, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.linear1 = MetaLinear(32*30, hidden1)
        self.relu1 = nn.ReLU(inplace=True)
        self.linear2 = MetaLinear(hidden1, output)
        # self.linear3 = MetaLinear(hidden2, output)
...
class Small_VCNN(MetaModule):
    def __init__(self, hidden1, output, k_size=3):
        super(Small_VCNN, self).__init__()
        assert k_size % 2 == 1
        padding = (k_size - 1) // 2
        self.conv1 = MetaConv1d(1, 16, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.conv2 = MetaConv1d(16, 16, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.linear1 = MetaLinear(16*2, hidden1)
        self.relu1 = nn.ReLU(inplace=True)
        self.linear2 = MetaLinear(hidden1, output)
        # self.linear3 = MetaLinear(hidden2, output)
...
class VCNN2(MetaModule):
    def __init__(self, hidden1, output, k_size=5, l=60):
...
class VCNN2_with_lv(MetaModule):
    def __init__(self, hidden1, output, k_size=3):
        super(VCNN2_with_lv, self).__init__()
        assert k_size % 2 == 1
        padding = (k_size - 1) // 2
        self.conv1 = MetaConv1d(1, 16, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.conv2 = MetaConv1d(16, 16, kernel_size=k_size, stride=1, padding=padding, bias=False)
        self.linear1 = MetaLinear(16*62, hidden1)
        self.relu1 = nn.ReLU(inplace=True)
        self.linear2 = MetaLinear(hidden1, output)
        # self.linear3 = MetaLinear(hidden2, output)
...
class VCNN_BCE(MetaModule):
    def __init__(self, hidden1, output):
        ...
        self.linear1 = MetaLinear(10*30, hidden1)
        self.relu1 = nn.ReLU(inplace=True)
        self.linear2 = MetaLinear(hidden1, output)
        # self.linear3 = MetaLinear(hidden2, output)
...
if __name__ == '__main__':
    a = torch.randn((16, 1, 125))
    a = to_var(a)
    vcnn = VCNN2_with_lv(16, 1)
    b = vcnn(a)
    print('get output')
# if __name__ == '__main__':
#
#
#     resnet = ResNet32_1d(1, num_blocks=[3, 3, 3]).cuda()
#     a = torch.randn((16, 1, 120))
#     a = to_var(a)
#     b = resnet(a)
#     total_trainable_params = sum(
#         p.numel() for p in resnet.params() if p.requires_grad)

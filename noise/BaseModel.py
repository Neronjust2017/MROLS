...
model_path = results_path + 'models/model_state_no_meta_corruptionProb{}_corruptionType{}_seed{}.pth'.format(args.corruption_prob, args.corruption_type, args.seed)
vnet_path = results_path + 'models/vnet_state_no_meta_corruptionProb{}_corruptionType{}_seed{}.pth'.format(args.corruption_prob, args.corruption_type, args.seed)
txt = results_path + 'results/result_no_meta_corruptionProb{}_corruptionType{}_seed{}.txt'.format(args.corruption_prob, args.corruption_type, args.seed)
...
def train(train_loader,train_meta_loader,model, vnet,optimizer_model,optimizer_vnet,epoch):
    ...
    for batch_idx, (inputs, targets, index) in enumerate(train_loader):
        ...
        # compute the cross entropy loss of the intensified prediction
        # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        with torch.no_grad():
            softmax = nn.Softmax(dim=1)
            probs = softmax(outputs)
            # probs_1 = copy.deepcopy(probs)

            probs = torch.where(probs<=0.2, probs * (0.1 / 0.2), probs)
            probs = torch.where(probs>=0.7, probs * (0.95 / 0.7), probs)

            # probs_1 = torch.where(probs_1<=0.2, probs_1 if probs_1 <= 0.05 else 0.05, probs_1)
            # probs_1 = torch.where(probs_1>=0.7, max(probs_1, 0.95), probs_1)
            
            for i in range(probs.shape[0]):
                probs[i] /= torch.sum(probs[i])
                # probs_1[i] /= torch.sum(probs_1[i])

            log_probs = torch.log(probs)
            cost_in = nn.NLLLoss(reduce=False)(log_probs, targets)
            cost_in_tmp = cost_in.cpu().numpy()
            losses2save_intensified.append(cost_in_tmp)

            # log_probs_1 = torch.log(probs_1)
            # cost_in_1 = nn.NLLLoss(reduce=False)(log_probs_1, targets)
            # cost_in_tmp_1 = cost_in_1.cpu().numpy()
            # losses2save_intensified_1.append(cost_in_tmp_1)
...
def main():
    best_acc = 0
    # np.save('targets_corruptionProb{}_corruptionType{}.npy'.format(args.corruption_prob, args.corruption_type), train_loader.dataset.train_labels)
    ...
    for epoch in range(args.epochs):
        ...
        indexes, losses, entropy, losses_intensified = train(train_loader,train_meta_loader,model, vnet,optimizer_model,optimizer_vnet,epoch)
    ...
    np.save(results_path + 'loss_sequence/losses_corruptionProb{}_corruptionType{}_seed{}.npy'.format(args.corruption_prob,
                                                                         args.corruption_type, args.seed), loss_recorder)
    np.save(results_path + 'loss_sequence/entropy_corruptionProb{}_corruptionType{}_seed{}.npy'.format(args.corruption_prob,
                                                                          args.corruption_type, args.seed), entropy_recorder)
    np.save(results_path + 'loss_sequence/losses(intensified)_corruptionProb{}_corruptionType{}_seed{}.npy'.format(args.corruption_prob,
                                                                                                  args.corruption_type,
                                                                                                  args.seed),loss_recorder_intensified)
    # np.save(results_path + 'loss_sequence/losses(intensified_1)_corruptionProb{}_corruptionType{}_seed{}.npy'.format(args.corruption_prob,
    #                                                                                                                  args.corruption_type,
    #                                                                                                                  args.seed),
    #         loss_recorder_intensified_1)
